...
from utils import mutual_diffdml_loss
    ...
    def train_epoch(self):
        ...
        for batch_idx, (inputs, labels) in enumerate(self.training_dataloader):
            ...
            with torch.autocast(self.device.type, enabled=self.amp):
                logits_a, predictions_a, loss_a, f1 = self.model_a(inputs, labels, diff=True)
                logits_b, predictions_b, loss_b, f2 = self.model_b(inputs, labels, diff=True)

                # soft targets
                loss, _, _, _ = mutual_diffdml_loss(
                    predictions_a, predictions_b, f1, f2, labels,
                    label_smoothing=0.1
                )

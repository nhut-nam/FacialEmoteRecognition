...
from test import get_model as test
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Batch size for training"
    )
    ...
    # net_1 = get_model(len(train_dataset.classes), opt.model_size, in_22k=opt.in_22k)
    net_1 = get_model(len(train_dataset.classes), opt.model_size, in_22k=opt.in_22k)
    from convnext import convnext_tiny
    net_2 = convnext_tiny(pretrained=True, num_classes=len(train_dataset.classes))
    net = test(net_1, net_2, len(train_dataset.classes))

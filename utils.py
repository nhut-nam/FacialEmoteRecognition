...
def bidirectional_kl_loss(p_logits, q_logits, T=1.0, reduction="batchmean", eps=1e-8):
    p_log_prob = F.log_softmax(p_logits / T, dim=1)
    q_log_prob = F.log_softmax(q_logits / T, dim=1)

    p_prob = p_log_prob.exp()
    q_prob = q_log_prob.exp()

    kl_pq = F.kl_div(p_log_prob, q_prob, reduction=reduction)
    kl_qp = F.kl_div(q_log_prob, p_prob, reduction=reduction)

    return (kl_pq + kl_qp) * (T * T) / 2.0  # /2 để giữ scale cân bằng
...
def diffdml_loss(logits_m, logits_w, feats_m, feats_w, targets,
    ...
    loss = ce_m + lambda_kl * kl + mu_div * div
    return loss, {"ce_m": ce_m.item(), "kl": kl.item(), "div": div.item()}

def mutual_diffdml_loss(
    logits_a, logits_b, feats_a, feats_b, targets,
    lambda_kl=0.1, mu_div=0.1, label_smoothing=0.0
):
    """
    logits_a, logits_b: (B, num_classes)
    feats_a, feats_b: feature maps/embeddings (B, C, H, W) or (B, D)
    targets: (B,)
    """

    # Cross-entropy cho từng model
    ce_a = F.cross_entropy(logits_a, targets, label_smoothing=label_smoothing)
    ce_b = F.cross_entropy(logits_b, targets, label_smoothing=label_smoothing)

    # Bidirectional KL giữa output distributions
    kl = bidirectional_kl_loss(logits_a, logits_b)

    # Cosine feature diversity
    div = feature_cosine_diversity(feats_a, feats_b)

    # Loss riêng cho từng model
    loss_a = ce_a + lambda_kl * kl + mu_div * div
    loss_b = ce_b + lambda_kl * kl + mu_div * div

    # Loss tổng để backward gộp
    loss = loss_a + loss_b
    ...
    return loss, loss_a, loss_b, log_dict
